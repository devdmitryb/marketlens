...
### Auth
Trivial single-password auth, not multi-user. `login.html` POSTs a password to `/api/login`; on match the server returns a static `APP_TOKEN` from env, stored in `sessionStorage`/`localStorage` as `ml_token`. All `/api/*` routes (except `/api/health` and `/api/login`) require this token via `x-app-token` header or `?token=` query param (see `auth()` middleware in `server/index.js`). If `APP_PASSWORD` is unset, auth is effectively disabled (dev mode).
